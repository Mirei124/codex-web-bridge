...
describe("Storage", () => {
  ...
  it("round-trips and updates a host PATH",()=>{storage=new Storage(":memory:");const host={id:"h",name:"h",hostname:"h",port:22,username:"u",hostKeySha256:"k",identityFile:"",pathEnv:"/custom/bin:/usr/bin:/bin",createdAt:1};storage.upsertHost(host);expect(storage.host("h")?.pathEnv).toBe(host.pathEnv);storage.upsertHost({...host,pathEnv:"/other/bin:/usr/bin:/bin"});expect(storage.host("h")?.pathEnv).toBe("/other/bin:/usr/bin:/bin");});
});

...
describe("remote command safety", () => {
  ...
  it("prepends a literal host path to command lookup and remote programs", async () => {
    ...
    const path = "/home/alice/.local/share/tmux/bin";
    const wrapped = withPrependedPath(remote, path);
    await wrapped.execute("command", ["-v", "tmux"]);
    await wrapped.execute("tmux", ["list-sessions"]);
    expect(remote.calls[0]).toEqual([
      "sh",
      ["-c", 'PATH=$1${PATH:+:$PATH}; export PATH; command -v "$2"', "cwb-path", path, "tmux"],
    ]);
    expect(remote.calls[1]).toEqual([
      "sh",
      ["-c", 'PATH=$1${PATH:+:$PATH}; export PATH; shift; exec "$@"', "cwb-path", path, "tmux", "list-sessions"],
    ]);
  });
  it("builds app server and viewer panes without interpolating user values", async () => {
    ...
    const session = await runtime.start("cwb_1", "/repo with spaces", 43123);
    const attached = await runtime.attachViewer(session, "/repo with spaces", "thread-1");
    ...
    expect(
      remote.calls.some((c) => c[1][0] === "set-option" && c[1].includes("@cwb-thread") && c[1].includes("thread-1")),
    ).toBe(true);
  });
  it("uses the absolute Codex path inside tmux panes instead of relying on the tmux server PATH", async () => {
    ...
    expect(remote.calls.find((call) => call[1][0] === "new-session")?.[1].at(-1)).toContain(
      "'check_for_update_on_startup=false'",
    );
    expect(remote.calls.find((call) => call[1][0] === "split-window")?.[1].at(-1)).toContain(
      "'check_for_update_on_startup=false'",
    );
  });
  it("accepts resolved programs when an SSH server omits the exit status", async () => {
    const remote = new FakeRemote();
    remote.commandLookupCode = null;
    const runtime = new TmuxCodexRuntime(remote, { runtimeDirectory: "/safe/runtime" });
    await runtime.checkPrerequisites();
  ...
  it("derives a private runtime directory from the remote SFTP home", async () => {
    const remote = new FakeRemote();
    await new TmuxCodexRuntime(remote).start("private", "/repo", 43123);
    expect(remote.calls.find((call) => call[0] === "mkdir")?.[1]).toContain(
      "/home/alice/.local/state/codex-web-bridge/runtime",
    );
    expect(remote.calls.some((call) => call[0] === "stat" && call[1][1] === "%u")).toBe(true);
  });
  it("rejects unsafe tmux names before issuing commands", async () => {
    const remote = new FakeRemote();
    await expect(new TmuxCodexRuntime(remote).start("bad;name", "/tmp", 4000)).rejects.toThrow("Invalid");
    expect(remote.calls).toHaveLength(0);
  });
  it("discovers an existing managed tmux session without replacing it", async () => {
    const remote = new FakeRemote();
    remote.sessionExists = true;
    const result = await new TmuxCodexRuntime(remote, { runtimeDirectory: "/safe/runtime" }).start(
      "existing",
      "/tmp",
      4000,
    );
    expect(result).toMatchObject({ appServerPane: "%1", viewerPane: "%2" });
    expect(remote.calls.some((call) => call[1][0] === "new-session")).toBe(false);
  });
  it("waits for the forwarded app-server port", async () => {
    const remote = new FakeRemote();
    remote.sessionExists = true;
  ...
  it("keeps one FIFO reader and captures a seed when reconnecting", async () => {
    ...
    expect(remote.streams[0].closed).toBe(true);
    expect(remote.calls.some((call) => call[1][0] === "capture-pane")).toBe(true);
  });
  it("sends terminal controls and escape sequences as tmux keys while pasting text through unique buffers", async () => {
    ...
    for (const value of [
      ...
      await runtime.sendKeys(session, value);
    await Promise.all([runtime.sendKeys(session, "hello"), runtime.sendKeys(session, "world")]);
    const keys = remote.calls.filter((call) => call[1][0] === "send-keys").map((call) => call[1].at(-1));
    expect(keys).toEqual([
      "C-c",
      "C-d",
      "BSpace",
    ...
    const buffers = remote.calls.filter((call) => call[1][0] === "set-buffer");
    expect(buffers.map((call) => call[1].at(-1))).toEqual(["hello", "world"]);
    expect(new Set(buffers.map((call) => call[1][2])).size).toBe(2);
  });
  ...
  it("kills the tmux session before removing its FIFO", async () => {
    const remote = new FakeRemote();
    await new TmuxCodexRuntime(remote, { runtimeDirectory: "/safe/runtime" }).stop("cwb_1");
    const kill = remote.calls.findIndex((call) => call[0] === "tmux" && call[1][0] === "kill-session");
    const remove = remote.calls.findIndex((call) => call[0] === "rm");

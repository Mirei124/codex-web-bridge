...
describe("remote command safety", () => {
  ...
  it("sends terminal controls and escape sequences as tmux keys while pasting text through unique buffers",async()=>{const remote=new FakeRemote(),runtime=new TmuxCodexRuntime(remote),session={name:"cwb_1",appServerPane:"%1",viewerPane:"%2",remotePort:4000,fifoPath:"/fifo"};for(const value of ["\x03","\x04","\x08","\t","\r","\x1b","\x1b[A","\x1b[B","\x1b[C","\x1b[D","\x1b[3~","\x1b[1;5A","\x1bx"])await runtime.sendKeys(session,value);await Promise.all([runtime.sendKeys(session,"hello"),runtime.sendKeys(session,"world")]);const keys=remote.calls.filter(call=>call[1][0]==="send-keys").map(call=>call[1].at(-1));expect(keys).toEqual(["C-c","C-d","BSpace","Tab","Enter","Escape","Up","Down","Right","Left","Delete","C-Up","M-x"]);const buffers=remote.calls.filter(call=>call[1][0]==="set-buffer");expect(buffers.map(call=>call[1].at(-1))).toEqual(["hello","world"]);expect(new Set(buffers.map(call=>call[1][2])).size).toBe(2);});
  it("reads the actual viewer pane dimensions",async()=>{const runtime=new TmuxCodexRuntime(new FakeRemote());await expect(runtime.dimensions({name:"cwb",appServerPane:"%1",viewerPane:"%2",remotePort:4000,fifoPath:"/fifo"})).resolves.toEqual({cols:93,rows:31});});

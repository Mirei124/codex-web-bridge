...
const DEFAULT_FONT_FAMILY =
  [
    ...
    .filter((family) => GlobalFonts.has(family))
    .map((family) => `"${family}"`)
    .join(", ") || "sans-serif";
...
export interface RenderedTerminalSnapshot {
  png: Buffer;
  text: string;
  cols: number;
  rows: number;
...
export class TerminalSnapshotRenderer {
  static readonly minCols = 20;
  static readonly maxCols = 240;
  static readonly minRows = 5;
  static readonly maxRows = 100;
  ...
  async render(ansi: string, options: TerminalSnapshotOptions): Promise<RenderedTerminalSnapshot> {
    ...
    try {
      ...
      context.font = `${options.fontSize ?? 14}px ${options.fontFamily ?? DEFAULT_FONT_FAMILY}`;
      const lines: string[] = [];
      for (let row = 0; row < rows; row++) {
        const line = terminal.buffer.active.getLine(startY + row);
        if (!line) {
          lines.push("");
          continue;
        }
        ...
        for (let column = 0; column < cols;) {
          ...
          if (background) {
            ...
            context.fillRect(x, y, CELL_WIDTH * width, CELL_HEIGHT);
          }
          const chars = cell.getChars();
          if (chars && chars !== " ") {
            context.fillStyle = foreground;
            context.font = `${cell.isBold() ? "bold " : ""}${options.fontSize ?? 14}px ${options.fontFamily ?? DEFAULT_FONT_FAMILY}`;
            context.fillText(chars, x, y + 1);
          }
          column += width;
        }
      }

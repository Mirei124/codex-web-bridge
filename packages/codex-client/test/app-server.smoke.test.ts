...
describe.skipIf(!enabled)("installed Codex app-server", () => {
  it("initializes over loopback WebSocket and lists threads without starting a turn", async (context) => {
    ...
    if (version.error && (version.error as NodeJS.ErrnoException).code === "ENOENT") {
      console.warn("SKIP: codex is not installed or not in PATH");
      return context.skip();
    }
    ...
    try {
      ...
      if (["EPERM", "EACCES"].includes((error as NodeJS.ErrnoException).code ?? "")) {
        console.warn("SKIP: sandbox does not permit loopback bind");
        return context.skip();
      }
      throw error;
    }
    ...
    let stderr = "";
    child = spawn("codex", ["app-server", "--listen", `ws://127.0.0.1:${port}`], {
      cwd: workingDirectory,
      stdio: ["ignore", "ignore", "pipe"],
    });
    child.stderr?.on("data", (chunk) => (stderr = (stderr + String(chunk)).slice(-8_000)));
    try {
      await waitForPort(port, child, 15_000);
    } catch (error) {
      if (/operation not permitted|permission denied/i.test(stderr)) {
        console.warn(`SKIP: sandbox rejected app-server bind: ${stderr.trim()}`);
        return context.skip();
      }
    ...
    const client = new CodexClient({
      ...
      clientInfo: { name: "cwb-smoke", version: "0.1.0" },
    });
    try {

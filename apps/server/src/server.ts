...
export async function buildServer(
  ...
  app.post<{ Body: { hostId: string; cwd: string; proxy?: string; prependPath?: string } }>(
    apiRoutes.threads,
    async (request, reply) => {
      ...
      try {
        const codexThreadId = await runtime.create(host, thread);
        storage.updateThread(id, { codexThreadId, status: "idle", updatedAt: Date.now() });
  ...
  app.post<{ Body: { hostId: string; codexThreadId: string; cwd?: string; proxy?: string; prependPath?: string } }>(
    ...
    async (request, reply) => {
      ...
      storage.createThread(thread);
      try {
        await runtime.resume(host, thread, request.body.codexThreadId);

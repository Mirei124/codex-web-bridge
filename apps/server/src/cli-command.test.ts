...
describe("LLM-oriented CLI command parsing", () => {
  ...
  it("parses bracketed IPv6 SSH targets", () => {
    expect(parseBusinessCommand(["host", "add", "u@[2001:db8::1]:2200", "--id", "v6", "--name", "IPv6"])).toMatchObject(
      { params: { username: "u", hostname: "2001:db8::1", port: 2200 } },
    );
  });

  it("derives a stable ID and name when only USER@HOST is supplied", () => {
    expect(parseBusinessCommand(["host", "add", "codex@Machine-A.Example"])).toMatchObject({
      params: { id: "codex-machine-a-example-22", name: "Machine-A.Example", username: "codex", port: 22 },
    });
  });
  ...
  it("supports explicit password clearing and rejects conflicting password input", () => {
    expect(parseBusinessCommand(["host", "add", "codex@machine-a", "--clear-password"])).toMatchObject({
      params: { clearPassword: true },
    });
    expect(() =>
      parseBusinessCommand(["host", "add", "codex@machine-a", "--clear-password", "--password-stdin"]),
  ...
  it.each([
    ...
    [
      ["request", "approve", "thread-1", "request-1"],
      "request.approve",
      { threadId: "thread-1", requestId: "request-1" },
    ],
    [
      ["request", "decline", "thread-1", "request-1"],
      "request.decline",
      { threadId: "thread-1", requestId: "request-1" },
    ],
  ...
  it.each([
    ...
    [
      ["request", "resolve", "thread-1", "request-1", "--input-json", '{"approved":true}'],
      "request.resolve",
      {
        threadId: "thread-1",
        requestId: "request-1",
        inputJson: '{"approved":true}',
      },
    ],
    [
      ["request", "answer", "thread-1", "request-1", "--input-file", "answers.json"],
      "request.answer",
      {
        threadId: "thread-1",
        requestId: "request-1",
        inputFile: "answers.json",
      },
    ],
  ...
  it.each([
    [
      ["thread", "create", "--host", "machine-a", "--cwd", "/work", "--prepend-path", "/thread/bin"],
      "thread.create",
      {
        hostId: "machine-a",
        cwd: "/work",
    ...
    [
      ["thread", "create", "--host", "machine-a", "--cwd", "/work", "--proxy", "http://proxy:8080"],
      "thread.create",
      {
        hostId: "machine-a",
        cwd: "/work",
    ...
    [
      ["thread", "send", "thread-1", "--text-file", "-"],
      "thread.send",
      {
        threadId: "thread-1",
        text: undefined,
    ...
    [
      ["terminal", "input", "thread-1", "--data", "\u001b[A"],
      "terminal.input",
      {
        threadId: "thread-1",
        data: "\u001b[A",
    ...
    [
      ["terminal", "input", "thread-1", "--data-file", "-"],
      "terminal.input",
      {
        threadId: "thread-1",
        data: undefined,
  ...
  it.each([
    ...
    expect(parseBusinessCommand(argv)).toMatchObject({
      method,

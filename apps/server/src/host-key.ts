...
export async function verifyHostKey(
  ...
  const supplied =
    typeof host.hostKeySha256 === "string" && /^SHA256:[A-Za-z0-9+/]{43}=?$/.test(host.hostKeySha256)
      ? host.hostKeySha256
      : undefined;
  ...
  if (!selected)
    throw new HostKeyError("SSH_HOST_KEY_SCAN_FAILED", `unable to read SSH host key for ${hostname}:${port}`, true);
  ...
  if (supplied && supplied !== fingerprint) {
    throw new HostKeyError(
      "HOST_KEY_CHANGED",
      `SSH host key does not match the confirmed fingerprint for ${hostname}:${port}`,
      false,
      { fingerprint },
    );
  }
  ...
  let existing = "";
  try {
    existing = await readFile(paths().knownHosts, "utf8");
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
  }
  const known = existing.split("\n").find((line) => line.startsWith(`${keyName} `));
  if (known && known !== entry)
    throw new HostKeyError("HOST_KEY_CHANGED", `SSH host key changed for ${hostname}:${port}`, false, { fingerprint });
  if (!accept && (requireConfirmation || !known)) {
    throw new HostKeyError("HOST_KEY_UNKNOWN", `SSH host key requires confirmation for ${hostname}:${port}`, false, {
      fingerprint,
    });
  }
  if (!known) {
...
function requiredString(value: Record<string, unknown>, name: string): string {
  ...
  if (typeof field !== "string" || !field)
    throw new HostKeyError("INVALID_ARGUMENT", `${name} must be a non-empty string`, false);
  return field;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code) =>
      code === 0 && stdout.trim()
        ? resolve(stdout)
        : reject(
            new HostKeyError("SSH_HOST_KEY_SCAN_FAILED", `unable to read SSH host key for ${hostname}:${port}`, true),
          ),
    );
  });
}

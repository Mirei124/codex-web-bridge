...
describe("CLI human renderer", () => {
  ...
  it("renders daemon results and generated credentials as prose", () => {
    expect(renderHuman({ state: "running", pid: 42, generatedPassword: "secret" }, { method: "start" })).toBe(
      "Daemon started (PID 42).\nDashboard password: secret",
    );
    expect(renderHuman({ state: "not_running" }, { method: "status" })).toBe("Daemon is not running.");
    expect(renderHuman({ daemonRestarted: true }, { method: "password.set" })).toBe(
  ...
  it("renders detail objects as readable key/value lines", () => {
    const rendered = renderHuman(
      { id: "thread-1", title: "Work", status: "waiting", hostId: "a", cwd: "/work" },
      { method: "thread.get" },
    );
  ...
  it("renders mutations from command context instead of dumping response objects", () => {
    expect(renderHuman({ completed: true }, { method: "thread.interrupt", params: { threadId: "thread-1" } })).toBe(
      "Thread thread-1 interrupted.",
    );
    expect(renderHuman({}, { method: "thread.delete", params: { threadId: "thread-1" } })).toBe(
      "Thread thread-1 deleted from Codex Web Bridge.",
    );
  ...
  it("renders streaming text without JSON decoration", () => {
    expect(
      renderHuman({ type: "message.delta", threadId: "t", messageId: "m", delta: "hello" }, { kind: "event" }),
    ).toBe("hello");
    expect(renderHuman({ type: "terminal.data", threadId: "t", data: "\\x1b[2J" }, { kind: "event" })).toBe("\\x1b[2J");
  ...
  it("renders errors with their stable code", () => {
    expect(renderHumanError({ code: "not_found", message: "thread missing" })).toBe(
      "Error: thread missing (not_found)",
    );
  });
});

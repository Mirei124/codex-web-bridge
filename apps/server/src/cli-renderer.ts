...
function daemonResult(method: string | undefined, data: Row): string | undefined {
  ...
  if (method === "start" || method === "restart") {
    const lines = [
      `Daemon ${data.state === "starting" ? "is starting" : "started"}${data.pid ? ` (PID ${scalar(data.pid)})` : ""}.`,
    ];
  ...
  if (method === "password.set") {
    return ["Dashboard password updated.", `Daemon restarted: ${scalar(data.daemonRestarted)}`].join("\n");
  }
}
...
export function renderHuman(data: unknown, context: RenderContext = {}): string {
  ...
  if (Array.isArray(data)) {
    ...
    return rows.length === data.length
      ? table(rows, context.method ? columns[context.method] : undefined)
      : data.map(scalar).join("\n");
  }
  ...
  if (
    ["thread.get", "thread.create", "thread.resume", "thread.restore", "thread.wait", "thread.watch"].includes(
      context.method ?? "",
    )
  ) {
  ...
  if (context.method === "host.get") {
    ...
    return labeled([
      ...
      ["Status", host.status],
      ["Host key", host.hostKeySha256],
      ["Identity file", host.identityFile],
    ]);
  }
  return details(data as Row);
}

export function humanEventMode(data: unknown, method?: string): "raw" | "line" | "stderr" | "ignore" {

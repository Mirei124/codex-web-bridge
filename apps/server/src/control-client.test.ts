...
async function listen(handler: (request: Record<string, unknown>, reply: (value: unknown) => void) => void): Promise<string> {
...
describe("daemon control transport", () => {
  ...
  it("forwards ordered stream events and resolves only on done", async () => {
    ...
    await expect(controlRequest(socketPath, "thread.watch", { id: "t" }, {
      stream: true,
  ...
  it("preserves daemon error codes and details for stable stderr/exit mapping", async () => {
    ...
    expect((error as ControlRequestError).controlError).toEqual({
      code: "not_found", message: "thread not found", details: { id: "missing" }, retryable: false,
    });
  });

  it.each([
    ["invalid JSON", (_request: Record<string, unknown>, reply: (value: unknown) => void) => reply("not-json"), "protocol_error"],
    ["wrong protocol version", (request: Record<string, unknown>, reply: (value: unknown) => void) =>
      reply({ version: 2, id: request.id, ok: true }), "protocol_error"],
    ["wrong request id", (_request: Record<string, unknown>, reply: (value: unknown) => void) =>
      reply({ version: 1, id: "other", ok: true }), "protocol_error"],
  ...
  it("reports a missing daemon socket as a retryable structured error", async () => {
    ...
    expect((error as ControlRequestError).controlError).toMatchObject({
      code: "daemon_unavailable", message: "daemon is not running", retryable: true,
    });
  });

  it("times out a silent daemon without switching to public HTTP", async () => {
    const socketPath = await listen(() => undefined);
    const error = await controlRequest(socketPath, "thread.wait", { id: "t" }, { timeoutMs: 10 }).catch(value => value);
    expect(error).toBeInstanceOf(ControlRequestError);

...
interface Active {
  hostId: string;
  ssh: SshConnection;
  runtime: TmuxCodexRuntime;
  session: RemoteSession;
  client: CodexClient;
  forward: { port: number; close(): Promise<void> };
...
export class HostRuntimeManager implements RuntimeManager {
  ...
  setHostPassword(hostId: string, password?: string): void {
    if (password) this.hostPasswords.set(hostId, password);
    else this.hostPasswords.delete(hostId);
  }
  hostStatus(hostId: string): "online" | "connecting" | "offline" {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      const suffix = randomUUID().replaceAll("-", "").slice(0, 16),
        thread: ThreadRecord = {
          id: `history-${suffix}`,
          hostId: host.id,
          tmuxSession: `cwb-history-${suffix}`,
      ...
      try {
        probe = await this.open(host, thread);
        return await this.historicalFrom(probe.client);
      } catch (error) {
        lastError = error;
      } finally {
        if (probe) await this.dispose(probe, { stopTmux: true });
      }
    }
  ...
  async reconnect(host: HostRecord, thread: ThreadRecord): Promise<void> {
    if (this.active.has(thread.id)) return;
    if (!thread.codexThreadId) throw new Error("thread has no Codex id");
    this.detached.delete(thread.id);
    this.events.emit("connectionGenerationChanged", { threadId: thread.id });
    this.cancelRetry(thread.id);
    const state = { host, thread, attempt: 0, cancelled: false };
    this.retries.set(thread.id, state);
    await this.tryReconnect(state);
  }
  async detach(threadId: string): Promise<void> {
    ...
    void this.dispose(active, { stopTmux: false });
  }
  async exit(thread: ThreadRecord, host?: HostRecord): Promise<void> {
    this.cancelRetry(thread.id);
    const active = this.active.get(thread.id);
    if (active) {
      this.active.delete(thread.id);
      await this.dispose(active, { stopTmux: true, requireStop: true });
      return;
    }
    if (!host) throw new Error("host is required to exit a disconnected thread");
    ...
    try {
      ...
      ).stop(thread.tmuxSession);
    } finally {
      ssh.close();
    }
  }
  async send(thread: ThreadRecord, text: string): Promise<string | undefined> {
    const active = this.must(thread.id);
    const result = (await active.client.startTurn(thread.codexThreadId!, text)) as { turn?: { id?: string } };
    active.turnId = result.turn?.id;
    active.hasRollout = true;
    await this.prepareTerminal(thread);
    return active.turnId;
  }
  async interrupt(thread: ThreadRecord): Promise<void> {
    const active = this.must(thread.id);
    if (active.turnId) await active.client.interruptTurn(thread.codexThreadId!, active.turnId);
  }
  async resolve(thread: ThreadRecord, requestId: string | number, value: unknown): Promise<void> {
    this.must(thread.id).client.respond(requestId, value as never);
  }
  async prepareTerminal(thread: ThreadRecord): Promise<void> {
    const active = this.must(thread.id);
    if (!active.hasRollout) throw new Error("terminal is unavailable until the first message is sent");
    if (active.session.viewerPane && active.stream) return;
    try {
      if (!active.session.viewerPane)
        active.session = await active.runtime.attachViewer(
          active.session,
          thread.workingDirectory,
      ...
      if (!active.stream) await this.attachTerminal(thread.id, active);
    } catch (error) {
      this.connectionLost(thread.id, active, "viewer attach failed");
      throw error;
    }
  }
  async terminalInput(thread: ThreadRecord, data: string): Promise<void> {
    await this.prepareTerminal(thread);
    const active = this.must(thread.id);
    await active.runtime.sendKeys(active.session, data);
  }
  async terminalSeed(thread: ThreadRecord): Promise<string> {
    const active = this.must(thread.id);
  ...
  async screenshot(thread: ThreadRecord): Promise<Buffer | undefined> {
    ...
    if (!active.hasRollout) return;
    await this.prepareTerminal(thread);
    const [ansi, size] = await Promise.all([
  ...
  async close(): Promise<void> {
    ...
    for (const [threadId, active] of [...this.active]) {
      this.active.delete(threadId);
      await this.dispose(active, { stopTmux: false });
    }
  }
  private async open(host: HostRecord, thread: ThreadRecord): Promise<Active> {
    if (!thread.remotePort) throw new Error("thread has no persisted app-server port");
    this.connecting.add(host.id);
    let ssh: SshConnection | undefined;
    let forward: { port: number; close(): Promise<void> } | undefined,
      client: CodexClient | undefined,
      runtime: TmuxCodexRuntime | undefined,
      session: RemoteSession | undefined;
    let tmuxCreated = false;
    try {
      ssh = await this.createSsh(host);
      await ssh.connect();
      runtime =
        this.options.runtimeFactory?.(ssh) ??
        new TmuxCodexRuntime(withPrependedPath(ssh, combinedPrependPath(host, thread)));
      await runtime.checkPrerequisites();
      const existed = await runtime.exists(thread.tmuxSession);
      ...
      console.error("[cwb:runtime] remote-ready", {
        threadId: thread.id,
        remotePort: thread.remotePort,
      });
      forward = await ssh.forwardRemotePort(thread.remotePort);
      console.error("[cwb:runtime] forward-ready", {
        threadId: thread.id,
        remotePort: thread.remotePort,
        localPort: forward.port,
      });
      const clientUrl = `ws://127.0.0.1:${forward.port}`;
      client =
        this.options.clientFactory?.(clientUrl) ??
        new CodexClient({
          url: clientUrl,
        });
  ...
  private async activate(host: HostRecord, thread: ThreadRecord, active: Active, withTerminal: boolean): Promise<void> {
    ...
    this.retries.set(thread.id, { host, thread, attempt: 0, cancelled: false });
    const lost = () => this.connectionLost(thread.id, active);
    active.client.on("transportError", lost);
    active.client.on("transportClose", lost);
    try {
      if (withTerminal) await this.attachTerminal(thread.id, active);
    } catch (error) {
      this.active.delete(thread.id);
      throw error;
    }
  }
  private async attachTerminal(threadId: string, active: Active): Promise<void> {
    await this.pipe(threadId, active);
    const lost = () => this.connectionLost(threadId, active);
    active.stream!.once("close", lost);
    active.stream!.once("error", lost);
  }
  private async pipe(threadId: string, active: Active): Promise<void> {
    active.stream = await active.runtime.terminalStream(active.session);
    active.stream.on("data", (data) =>
      this.events.emit("event", { threadId, type: "terminal", payload: data.toString("utf8") } satisfies RuntimeEvent),
    );
  }
  private connectionLost(threadId: string, active: Active, reason?: string): void {
    if (this.closing || this.active.get(threadId) !== active) return;
    this.events.emit("connectionGenerationChanged", { threadId, reason });
    this.events.emit("connectionLost", { threadId, reason });
    this.active.delete(threadId);
    void this.dispose(active, { stopTmux: false }).finally(() => {
      const state = this.retries.get(threadId);
      if (state && !state.cancelled) this.scheduleRetry(state);
    });
  }
  private async tryReconnect(state: RetryState): Promise<void> {
    if (state.cancelled || this.closing) return;
    try {
      await this.resume(state.host, state.thread, state.thread.codexThreadId!);
      state.attempt = 0;
    } catch {
      this.scheduleRetry(state);
    }
  }
  private scheduleRetry(state: RetryState): void {
    if (state.cancelled || this.closing || state.timer) return;
    const limit = this.options.retryLimit ?? 8;
    if (state.attempt >= limit) {
      this.events.emit("reconnectFailed", { threadId: state.thread.id, attempts: state.attempt });
      return;
    }
    const delay = Math.min((this.options.retryBaseMs ?? 500) * 2 ** state.attempt++, this.options.retryMaxMs ?? 30_000);
    state.timer = setTimeout(() => {
      state.timer = undefined;
      void this.tryReconnect(state);
    }, delay);
    state.timer.unref?.();
  }
  ...
  private async dispose(active: Active, options: { stopTmux: boolean; requireStop?: boolean }): Promise<void> {
    active.stream?.removeAllListeners();
    active.stream?.close();
    active.client.removeAllListeners("transportError");
    active.client.removeAllListeners("transportClose");
    active.client.close();
    await active.forward.close().catch(() => undefined);
    try {
      if (options.stopTmux) {
        const stopping = active.runtime.stop(active.session.name);
        if (options.requireStop) await stopping;
        else await stopping.catch(() => undefined);
      }
    } finally {
      active.ssh.close();
    }
  ...
  private async historicalFrom(
    client: CodexClient,
  ): Promise<Array<{ id: string; title?: string; cwd?: string; updatedAt?: string }>> {
  ...
  private async createSsh(host: HostRecord): Promise<SshConnection> {
    ...
    const config: ConstructorParameters<typeof SshConnection>[0] = {
      host: host.hostname,
      port: host.port,
    ...
    if (password) config.password = password;
      ...
      for (const name of ["id_ed25519", "id_ecdsa", "id_rsa"]) {
        try {
          config.privateKey = await readFile(join(homedir(), ".ssh", name));
          break;
        } catch (error) {
  ...
  private must(threadId: string): Active {
    const value = this.active.get(threadId);
...
export function opensshSha256ToHex(value: string): string {
  if (!value.startsWith("SHA256:")) throw new Error("host key fingerprint must use OpenSSH SHA256:base64 format");
  const encoded = value.slice(7);
  if (!/^[A-Za-z0-9+/]+={0,2}$/.test(encoded)) throw new Error("invalid OpenSSH host key fingerprint");
  const bytes = Buffer.from(encoded, "base64");
  if (bytes.length !== 32) throw new Error("invalid SHA256 host key fingerprint length");
  return bytes.toString("hex");
}
function combinedPrependPath(host: HostRecord, thread: ThreadRecord): string | undefined {
  return [thread.prependPath, host.prependPath].filter(Boolean).join(":") || undefined;
}

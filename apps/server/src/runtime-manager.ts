...
export class HostRuntimeManager implements RuntimeManager {
  ...
  async send(thread: ThreadRecord, text: string): Promise<string | undefined> { const active = this.must(thread.id); const result = await active.client.startTurn(thread.codexThreadId!, text) as { turn?: { id?: string } }; active.turnId = result.turn?.id;await this.prepareTerminal(thread);return active.turnId; }
  ...
  async screenshot(thread: ThreadRecord): Promise<Buffer | undefined> { await this.prepareTerminal(thread);const active=this.must(thread.id),ansi=await active.runtime.capture(active.session);return (await new TerminalSnapshotRenderer().render(ansi,{cols:120,rows:40})).png; }

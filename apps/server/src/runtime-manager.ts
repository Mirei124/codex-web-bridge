...
import { SshConnection, TmuxCodexRuntime, TerminalSnapshotRenderer, withRemotePath, type RemoteSession, type CommandStream } from "@cwb/remote-runtime";
...
export class HostRuntimeManager implements RuntimeManager {
  ...
  async exit(thread: ThreadRecord, host?:HostRecord): Promise<void> { this.cancelRetry(thread.id);const active=this.active.get(thread.id);if(active){this.active.delete(thread.id);await this.dispose(active,{stopTmux:true,requireStop:true});return;}if(!host)throw new Error("host is required to exit a disconnected thread");const ssh=await this.createSsh(host);try{await ssh.connect();await (this.options.runtimeFactory?.(ssh)??new TmuxCodexRuntime(withRemotePath(ssh,host.pathEnv))).stop(thread.tmuxSession);}finally{ssh.close();} }

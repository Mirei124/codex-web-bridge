...
export class HostRuntimeManager implements RuntimeManager {
  ...
  async resume(host: HostRecord, thread: ThreadRecord, codexThreadId: string): Promise<void> {
    const active = await this.open(host, thread);
    try{await active.client.resumeThread(codexThreadId, { cwd: thread.workingDirectory });active.session = await active.runtime.attachViewer(active.session, thread.workingDirectory, codexThreadId);await this.activate(host,thread,active,true);}
    catch(error){await this.dispose(active,{stopTmux:active.tmuxCreated});throw error;}
  }
  ...
  async exit(thread: ThreadRecord, host?:HostRecord): Promise<void> { this.cancelRetry(thread.id);const active=this.active.get(thread.id);if(active){this.active.delete(thread.id);await this.dispose(active,{stopTmux:true,requireStop:true});return;}if(!host)throw new Error("host is required to exit a disconnected thread");const ssh=await this.createSsh(host);try{await ssh.connect();await (this.options.runtimeFactory?.(ssh)??new TmuxCodexRuntime(ssh)).stop(thread.tmuxSession);}finally{ssh.close();} }
  ...
  async terminalSeed(thread:ThreadRecord):Promise<string>{const active=this.must(thread.id);return active.runtime.capture(active.session,1000);}
  async screenshot(thread: ThreadRecord): Promise<Buffer | undefined> { const active=this.must(thread.id),ansi=await active.runtime.capture(active.session);return (await new TerminalSnapshotRenderer().render(ansi,{cols:120,rows:40})).png; }

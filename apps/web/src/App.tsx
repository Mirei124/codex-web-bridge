...
export default function App() {
  ...
  const [hostDialog, setHostDialog] = useState<HostSummary | "new">(); const [navigationOpen, setNavigationOpen] = useState(false); const [exiting, setExiting] = useState(false); const [operationError, setOperationError] = useState("");
  ...
  return <div className="shell"><aside className={navigationOpen ? "open" : ""}><div className="sidebar-heading"><div className="brand">Codex Bridge</div><button className="mobile-close secondary" aria-label="关闭导航" onClick={() => setNavigationOpen(false)}>×</button></div><div className="toolbar"><button onClick={() => setDialog("create")}>＋ 新会话</button><button className="secondary" onClick={() => setDialog("resume")}>恢复</button></div><div className="section-title"><h3>主机</h3><button className="icon-button secondary" aria-label="添加主机" onClick={() => setHostDialog("new")}>＋</button></div>{hosts.map(h => <button className="host" key={h.id} onClick={() => setHostDialog(h)}><i className={h.status} /><span>{h.name}</span><small>{h.address}</small></button>)}<h3>会话</h3><nav>{threads.map(t => <button className={selected?.id === t.id ? "active" : ""} key={t.id} onClick={() => void selectThread(t)}><span>{t.title}</span><small>{t.status} · {t.cwd}</small></button>)}</nav><button className="logout" onClick={() => void api.logout().then(() => setAuthenticated(false))}>退出登录</button></aside>{navigationOpen && <button className="navigation-backdrop" aria-label="关闭导航" onClick={() => setNavigationOpen(false)} />}<main className="workspace"><button className="mobile-nav-toggle secondary" onClick={() => setNavigationOpen(true)}>☰ 主机与会话</button>{selected ? <><header><div><h1>{selected.title}</h1><p>{selected.cwd} · {selected.status}</p></div><div className="actions"><button className={tab === "chat" ? "" : "secondary"} onClick={() => setTab("chat")}>对话</button><button className={tab === "terminal" ? "" : "secondary"} onClick={() => setTab("terminal")}>终端</button><button className="danger" disabled={selected.status === "exited" || exiting} onClick={() => void exitSelected()}>{exiting ? "退出中…" : selected.status === "exited" ? "已退出" : "退出会话"}</button></div></header>{operationError && <div className="operation-error error">{operationError}</div>}{tab === "chat" ? <><div className="conversation">{selected.messages.map(m => <article className={m.role} key={m.id}><label>{m.role === "assistant" ? "Codex" : m.role === "user" ? "你" : "系统"}{m.streaming && <em>生成中</em>}</label><div>{m.text}</div></article>)}{selected.pendingRequests.map(r => <RequestCard key={r.requestId} request={r} onResolve={v => void resolve(r, v)} />)}</div><form className="composer" onSubmit={send}><textarea disabled={selected.status === "exited"} value={draft} onChange={e => setDraft(e.target.value)} placeholder={selected.status === "exited" ? "该会话已退出" : "继续对话…"} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); e.currentTarget.form?.requestSubmit(); } }} /><div>{selected.status === "running" && <button type="button" className="danger" onClick={() => void api.interrupt(selected.id)}>中断</button>}<button disabled={selected.status === "exited" || !draft.trim()}>发送</button></div></form></> : <div className="terminal-panel"><div className="terminal-toolbar"><span>{selected.terminal.connected ? "已连接" : "未连接"} · {selected.terminal.takeover ? `接管中${selected.terminal.owner ? ` (${selected.terminal.owner})` : ""}` : "只读"}</span><div className="actions"><button className="secondary" onClick={() => setScreenshot(api.screenshotUrl(selected.id))}>截图</button><button disabled={selected.status === "exited"} className={selected.terminal.takeover ? "danger" : ""} onClick={() => void api.takeover(selected.id, !selected.terminal.takeover)}>{selected.terminal.takeover ? "结束接管" : "显式接管"}</button></div></div><Terminal data={terminalData} writable={selected.terminal.takeover} onInput={terminalInput} />{screenshot && <div className="screenshot"><button onClick={() => setScreenshot(undefined)}>关闭</button><img src={screenshot} alt="终端截图" /></div>}</div>}</> : <section className="empty"><h1>选择一个会话</h1><p>查看 Codex 输出，继续对话或恢复历史线程。</p></section>}</main>{dialog && <CreateDialog hosts={hosts} mode={dialog} onClose={() => setDialog(undefined)} onDone={thread => { setDialog(undefined); setThreads(old => [thread, ...old]); setSelected(thread); setNavigationOpen(false); }} />}{hostDialog && <HostDialog host={hostDialog === "new" ? undefined : hostDialog} onClose={() => setHostDialog(undefined)} onSaved={() => { setHostDialog(undefined); void load(); }} />}</div>;
}

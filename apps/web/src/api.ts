...
export const api = {
  session: async () => {
    const session = await request<SessionResponse>(apiRoutes.session);
    csrfToken = session.csrfToken;
    return session;
  },
  login: async (body: LoginRequest) => {
    const session = await request<SessionResponse>(apiRoutes.login, { method: "POST", body: JSON.stringify(body) });
    csrfToken = session.csrfToken;
  },
  logout: async () => {
    await request<void>(apiRoutes.logout, { method: "POST" });
    csrfToken = undefined;
    window.dispatchEvent(new Event("cwb-auth-changed"));
  },
  hosts: () => request<HostSummary[]>(apiRoutes.hosts),
  saveHost: (body: HostConfig) =>
    request<{ id: string }>(apiRoutes.hosts, { method: "POST", body: JSON.stringify(body) }),
  ...
  createThread: (body: CreateThreadRequest) =>
    request<ThreadDetail>(apiRoutes.threads, { method: "POST", body: JSON.stringify(body) }),
  resumeThread: (body: ResumeThreadRequest) =>
    request<ThreadDetail>(apiRoutes.resumeThread, { method: "POST", body: JSON.stringify(body) }),
  ...
  sendMessage: (id: string, body: SendMessageRequest) =>
    request<void>(`${apiRoutes.threads}/${encodeURIComponent(id)}/messages`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  interrupt: (id: string) =>
    request<void>(`${apiRoutes.threads}/${encodeURIComponent(id)}/interrupt`, { method: "POST" }),
  resolve: (id: string, requestId: string, body: ResolveRequest) =>
    request<void>(`${apiRoutes.threads}/${encodeURIComponent(id)}/requests/${encodeURIComponent(requestId)}`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  takeover: (id: string, enabled: boolean) =>
    request<void>(`${apiRoutes.threads}/${encodeURIComponent(id)}/terminal/takeover`, {
      method: "POST",
      body: JSON.stringify({ enabled }),
    }),
  terminalInput: (id: string, data: string) =>
    request<void>(`${apiRoutes.threads}/${encodeURIComponent(id)}/terminal/input`, {
      method: "POST",
